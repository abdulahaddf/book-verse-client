...
const UserChats = () => {
  ...
  return (
    <div className="px-5 md:px-20 lg:px-20 w-4/5 mx-auto ">
      <h1 className="dashboard-heading">All Your Chats</h1>
      {allChats?.map((a) => (
        <div key={a?._id}>
          <button onClick={() => routeHandler(a)} className="w-full">
            <div className="my-2 p-[5px] py-[15px]  bg-gray-50  space-y-3 overflow-hidden rounded-md hover:rounded-xl  border-b-[5px]  border-r-[2px] hover:bg-gray-200 flex ">
              ...
              >
                {uniqueUsersData.map((userData) => {
                  ...
                  if (userData?.email === otherUserEmail) {
                    return (
                      <p
                        key={userData?.email}
                        className="text-[20px]    font-[500]  text-gray-600"
                      >
                ...
                {a?.chat[a?.chat.length - 1].email === user?.email && (
                  <p className="text-[15px] font-[400] text-gray-600">
