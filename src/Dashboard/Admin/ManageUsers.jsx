...
const ManageUsers = () => {
    ...
    return (
        <div className="w-full h-full ps-4 lg:p-4 md:mt-6">
            <h3 className="text-4xl font-bold text-center">Total Users: {users.length}</h3>
          ...
          <tbody>
            {users.map((user, index) => (
              <tr key={user._id}>
               ...
               <td ><button onClick={() => handleDelete(user)} className="w-full"><MdDeleteForever className=" text-4xl  text-red hover:text-blue-500" ></MdDeleteForever></button></td>
              </tr>
            ))}
          </tbody>
        </table>
